...
server <- function(input, output, session){
  ...
  output$map <- renderLeaflet({ show_map(plotBorders) })
  
  # Event that select the plot in the map
  observeEvent(input$map_shape_click, {
    
    # Color the plot
    click_map(input, selected_plot, plotBorders)

  })
  
  # Show information on box close to map
    output$infoBox <- renderUI({  info_box_render(static_data, selected_plot) })
    
    # Show information on static cards
